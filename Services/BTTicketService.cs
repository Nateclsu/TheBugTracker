...
namespace Phoenix.Services
{
    public class BTTicketService : IBTTicketService
    {
        ...
        public async Task<int?> LookupTicketPriorityIdAsync(string priorityName)
        {
            try
            {
                TicketPriority priority = await _context.TicketPriorities.FirstOrDefaultAsync(priority => priority.Name == priorityName);
                return priority?.Id;
            }
            catch (Exception)
            {
        ...
        public async Task<int?> LookupTicketStatusIdAsync(string statusName)
        {
            try
            {
                TicketStatus status = await _context.TicketStatuses.FirstAsync(status => status.Name == statusName);
                return status?.Id;
            }
            catch (Exception)
            {
